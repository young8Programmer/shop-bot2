...
export class BotController {
  ...
  private readonly adminId: number = 5661241603;
  ...
  private getLocalizedField(field: string, lang: string): string {
  ...
  private initializeBot() {
    ...
    this.bot.onText(/\/start/, async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('errors.general', this.DEFAULT_LANGUAGE));
      }
    });

    // Handle all messages (registration, support, admin actions)
      ...
      try {
        ...
        if (ctx.session.step === 'register_name' && msg.text) {
          if (!user) {
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.welcome', this.DEFAULT_LANGUAGE), {
              reply_markup: {
                inline_keyboard: [
        ...
        // Handle support messages
          ...
          await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.support_message_sent', lang));
          ctx.session.step = undefined;
        }
        // Handle admin actions
        else if (ctx.from.id === this.adminId) {
          if (ctx.session.step === 'add_product_name' && msg.text) {
            ctx.session.product = { nameUz: msg.text };
            ctx.session.step = 'add_product_desc';
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_desc_uz', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_name_ru', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_desc_ru', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_name_en', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_desc_en', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_price', lang));
          } else if (ctx.session.step === 'add_product_price' && msg.text) {
            const price = parseInt(msg.text, 10);
            if (isNaN(price) || price <= 0) {
              await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('errors.invalid_price', lang));
              return;
            }
            ...
            const buttons = categories.map((cat) => [{
              text: cat[this.getLocalizedField('name', lang)],
              callback_data: `set_category_${cat.id}`
            }]);
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.select_category', lang), {
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.broadcast_sent', lang));
            ctx.session.step = undefined;
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_category_name_ru', lang));
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_category_name_en', lang));
            ...
            await this.showCategoryManagement(ctx, lang);
            ctx.session.step = undefined;
        ...
        const lang = (await this.botService.getUser(ctx.from.id.toString()))?.language || this.DEFAULT_LANGUAGE;
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('errors.general', lang));
      }
    });

    // Handle all callback queries (language selection, categories, admin actions)
    this.bot.on('callback_query', async (callbackQuery: TelegramBot.CallbackQuery) => {
      ...
      try {
        ...
        if (callbackQuery.data?.startsWith('lang_')) {
          const newLang = callbackQuery.data.split('_')[1];
        ...
        // Category selection for products
          ...
          ctx.session.page = 0;
          await this.showProducts(ctx, categoryId, lang);
          await this.bot.answerCallbackQuery(callbackQuery.id);
        }
        ...
        // Delivery options
          ...
          await this.showPaymentOptions(ctx, lang);
          await this.bot.answerCallbackQuery(callbackQuery.id);
        }
        // Payment options
          ...
          if (!cart.length) {
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.cart_empty', lang));
            await this.bot.answerCallbackQuery(callbackQuery.id);
            return;
          }
          ...
          await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.order_placed', lang));
        ...
        // Admin actions
          ...
          if (data === 'manage_products') {
            await this.showProductManagement(ctx, lang);
          } else if (data === 'manage_categories') {
            await this.showCategoryManagement(ctx, lang);
          } else if (data === 'statistics') {
            await this.showStatistics(ctx, lang);
          } else if (data === 'broadcast') {
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.broadcast_message', lang));
            ctx.session.step = 'broadcast';
          } else if (data === 'view_users') {
            await this.showUsers(ctx, lang);
          } else if (data?.startsWith('add_product')) {
            ctx.session.step = 'add_product_name';
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_product_name_uz', lang));
          } else if (data?.startsWith('edit_product_')) {
            const productId = parseInt(data.split('_')[2], 10);
            ctx.session.productId = productId;
            await this.showEditProduct(ctx, productId, lang);
            ...
            await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_category_name_uz', lang));
          } else if (data?.startsWith('edit_category_')) {
            const categoryId = parseInt(data.split('_')[2], 10);
            ctx.session.categoryId = categoryId;
            await this.showEditCategory(ctx, categoryId, lang);
    ...
    this.bot.onText(/\/search (.+)/, async (msg: TelegramBot.Message, match: RegExpExecArray) => {
      ...
      try {
        ...
        if (!products.length) {
          await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.no_products_found', lang));
          return;
        }
        let message = await this.getTranslation('messages.found_products', lang) + '\n';
        const nameField = this.getLocalizedField('name', lang);
        products.forEach((product, index) => {
          message += `${index + 1}. ${product[nameField]} - ${product.price} UZS\n/add_${product.id}\n`;
        });
        ...
        const lang = (await this.botService.getUser(ctx.from.id.toString()))?.language || this.DEFAULT_LANGUAGE;
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('errors.search', lang));
      }
    });
    ...
    this.bot.onText(/\/add_(\d+)/, async (msg: TelegramBot.Message, match: RegExpExecArray) => {
      ...
      try {
        ...
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.added_to_cart', lang));
        this.sessions[ctx.from.id] = ctx.session;
      } catch (error) {
    ...
    this.bot.onText(/\/remove_(\d+)/, async (msg: TelegramBot.Message, match: RegExpExecArray) => {
      ...
      try {
        ...
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.removed_from_cart', lang));
        this.sessions[ctx.from.id] = ctx.session;
      } catch (error) {
    ...
    this.bot.onText(/\/set_quantity_(\d+) (\d+)/, async (msg: TelegramBot.Message, match: RegExpExecArray) => {
      ...
      try {
        ...
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('messages.quantity_updated', lang));
        this.sessions[ctx.from.id] = ctx.session;
      } catch (error) {
    ...
    this.bot.onText(/Savat|Корзина|Cart/, async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        const nameField = this.getLocalizedField('name', lang);
        for (const item of cart) {
          message += `${item.product[nameField]} - ${item.quantity} ${await this.getTranslation('common.units', lang)} - ${item.product.price * item.quantity} UZS\n/remove_${item.product.id} | /set_quantity_${item.product.id} <${await this.getTranslation('common.quantity', lang)}>\n`;
        }
    ...
    this.bot.onText(/\/place_order/, async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        if (!user) throw new Error('Foydalanuvchi topilmadi');
        const lang = user.language || this.DEFAULT_LANGUAGE;
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_phone', lang), {
          reply_markup: {
            keyboard: [[{ text: await this.getTranslation('buttons.send_phone', lang), request_contact: true }]],
    ...
    this.bot.on('contact', async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        if (ctx.session.step === 'get_phone') {
          ...
          await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.enter_address', lang), {
            reply_markup: {
              keyboard: [[{ text: await this.getTranslation('buttons.send_location', lang), request_location: true }]],
              resize_keyboard: true,
              one_time_keyboard: true,
            },
          });
    ...
    this.bot.on('location', async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        if (ctx.session.step === 'get_location') {
          ...
          await this.showDeliveryOptions(ctx, lang);
    ...
    this.bot.onText(/Qo‘llab-quvvatlash|Поддержка|Support/, async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        if (!user) throw new Error('Foydalanuvchi topilmadi');
        const lang = user.language || this.DEFAULT_LANGUAGE;
        await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.support_message', lang));
    ...
    this.bot.onText(/\/admin/, async (msg: TelegramBot.Message) => {
      ...
      try {
        ...
        if (ctx.from.id !== this.adminId) {
          await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('errors.no_admin_access', lang));
          return;
        }
        await this.sendAdminMenu(ctx, lang);
        this.sessions[ctx.from.id] = ctx.session;
      } catch (error) {
  ...
  private async showProducts(ctx: CustomContext, categoryId: number, lang: string) {
    try {
      ...
      paginatedProducts.forEach((product) => {
        message += `${product[nameField]} - ${product.price} UZS\n/add_${product.id}\n`;
      });
  ...
  private async showEditProduct(ctx: CustomContext, productId: number, lang: string) {
    try {
      ...
      await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.edit_product_name', lang, { current: product[nameField] }), {
        reply_markup: { force_reply: true },
      });
  ...
  private async showEditCategory(ctx: CustomContext, categoryId: number, lang: string) {
    try {
      ...
      await this.bot.sendMessage(ctx.chat.id, await this.getTranslation('prompts.edit_category_name', lang, { current: category[nameField] }), {
        reply_markup: { force_reply: true },
      });
